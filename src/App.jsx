...
const XC={
  ...
  lotti:[{key:"lotto",label:"Lotto"},{key:"imballo",label:"Imballo"},{key:"desc1",label:"Tipo"},{key:"desc3",label:"Calibro"},{key:"q_iniz",label:"Q.Iniziale"},{key:"mov",label:"Movimentato"},{label:"Disponibile",gv:r=>dsp(r)},{key:"magazzino",label:"Magazzino"},{key:"contratto",label:"Contratto"}],
...
function DashboardPage({lotti,contratti,goPage}){
  ...
  return <div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:28}}>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:20}}>{Object.entries(byCal).sort((a,b)=>b[1]-a[1]).map(([cal,kg])=>{const p=tot>0?kg/tot*100:0;return <div key={cal} onClick={()=>drill("calibro",cal)} style={{cursor:"pointer",background:"#faf8f4",borderRadius:10,padding:"12px 16px",textAlign:"center",border:"1px solid "+C.bd,transition:"all .2s",flex:"1 1 80px"}} onMouseEnter={e=>{e.currentTarget.style.borderColor=C.bdH}} onMouseLeave={e=>{e.currentTarget.style.borderColor=C.bd}}><div style={{fontSize:18,fontWeight:800,color:C.acc,fontFamily:"'DM Mono',monospace"}}>{cal}</div><div style={{fontSize:16,fontWeight:700,marginTop:6,fontFamily:"'DM Mono',monospace"}}>{kg.toLocaleString()}</div><div style={{fontSize:11,color:C.tM,marginTop:2}}>{p.toFixed(0)}%</div></div>})}</div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:10}}>{openC.map((c,i)=>{const res=c.qta_tot-c.qta_evasa;const p=c.qta_evasa/c.qta_tot*100;return <div key={c.id} style={{background:"#faf8f4",borderRadius:10,padding:14,border:"1px solid "+C.bd,animation:`slideIn .3s ${0.5+i*.04}s both`}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontWeight:700,color:C.acc,fontSize:13}}>N.{c.id} — {c.cliente}</span><span style={{fontSize:11,color:C.tM}}>{fmtD(c.scadenza)}</span></div><div style={{display:"flex",justifyContent:"space-between",marginBottom:6,fontSize:12}}><span style={{color:C.tM}}>Residuo: <strong style={{color:C.o}}>{res.toLocaleString()} kg</strong></span><span style={{color:C.tM,fontFamily:"'DM Mono',monospace"}}>{p.toFixed(0)}%</span></div><div style={{height:4,background:"#eee",borderRadius:2,overflow:"hidden"}}><div style={{height:"100%",width:p+"%",background:p>=100?C.g:C.acc,borderRadius:2}}/></div></div>})}</div></GCard>
  </div>;
}
...
function MovimentiPage({lotti,contratti,movimenti,reload,isAdm}){
  ...
  return <div>
    ...
    <div style={{display:"flex",gap:0,marginBottom:20,borderRadius:10,border:"1px solid "+C.bd,overflow:"hidden",background:C.sf}} data-no-print>{[{v:"ENTRATA",color:C.g,sub:"Nuovo lotto"},{v:"USCITA",color:C.r,sub:"Seleziona e preleva"},{v:"TRASFERIMENTO",color:C.b,sub:"Sposta tra magazzini"}].map(t=><div key={t.v} onClick={()=>chTipo(t.v)} style={{flex:1,padding:"14px 20px",cursor:"pointer",textAlign:"center",background:tipo===t.v?"#faf8f4":"transparent",borderBottom:tipo===t.v?"3px solid "+t.color:"3px solid transparent",transition:"all .15s"}}><div style={{fontSize:14,fontWeight:700,color:tipo===t.v?t.color:C.tM}}>{t.v}</div><div style={{fontSize:11,color:C.tM,marginTop:2}}>{t.sub}</div></div>)}</div>
    ...
    <GCard style={{marginBottom:24}} data-no-print>
      ...
      :<>{sel?<><h3 style={{fontSize:14,color:C.b,margin:"0 0 16px",fontWeight:700}}>Trasferimento</h3><div style={{display:"flex",gap:12,alignItems:"flex-end",flexWrap:"wrap"}}><Inp label="Data" type="date" value={form.data} onChange={v=>setForm({...form,data:v})}/><div><div style={{fontSize:10,color:C.tD,marginBottom:4}}>DA</div><div style={{padding:"8px 12px",background:"#eee",border:"1px solid "+C.bd,borderRadius:8,color:C.tD,fontSize:13}}>{sel.magazzino}</div></div><Sel label="A" value={form.newMag} onChange={v=>setForm({...form,newMag:v})} options={MAGS.filter(v=>v!==sel.magazzino).map(v=>({value:v,label:v}))}/><Inp label={"Qta (max "+sel.d+" kg)"} type="number" value={form.qta} onChange={v=>setForm({...form,qta:v})} placeholder={String(sel.d)}/><Btn small onClick={()=>setForm(f=>({...f,qta:String(sel.d)}))}>Tutto</Btn></div></>:!showPk&&<div style={{textAlign:"center",padding:20}}><Btn primary onClick={()=>setShowPk(true)}>Seleziona lotto</Btn></div>}</>}
...
function LottiPage({lotti,reload,isAdm}){
  ...
  return <div>
    ...
    <Tbl cols={[{key:"lotto",label:"Lotto"},{key:"imballo",label:"Imballo"},{label:"Tipo",render:r=><Badge color={TC[r.desc1]||C.acc} bg={(TC[r.desc1]||C.acc)+"18"}>{r.desc1}</Badge>},{key:"desc3",label:"Cal."},{label:"Q.Iniz",render:r=><span style={{fontFamily:"'DM Mono',monospace"}}>{r.q_iniz?.toLocaleString()}</span>},{label:"Mov.",render:r=><span style={{fontFamily:"'DM Mono',monospace",color:r.mov>0?C.r:C.tM}}>{r.mov?.toLocaleString()}</span>},{label:"Disp.",render:r=>{const d=dsp(r);return <span style={{fontFamily:"'DM Mono',monospace",fontWeight:700,color:d>0?C.g:C.r}}>{d.toLocaleString()}</span>}},{label:"Stato",render:r=>dsp(r)>0?<Badge color={C.g} bg={C.gD}>Disp.</Badge>:<Badge color={C.tM} bg="#eee">Esaurito</Badge>},{key:"magazzino",label:"Mag."},{key:"contratto",label:"Contr."},...(isAdm?[{label:"",render:r=><div style={{display:"flex",gap:6}}><button onClick={e=>{e.stopPropagation();openEdit(r)}} style={{background:"none",border:"none",color:C.acc,cursor:"pointer",fontSize:12,fontWeight:600}}>Mod</button>{dId===r.id?<span style={{display:"flex",gap:4}}><button onClick={e=>{e.stopPropagation();doDel(r.id)}} style={{background:C.r,border:"none",color:"#fff",cursor:"pointer",fontSize:11,padding:"3px 8px",borderRadius:6,fontWeight:700}}>Si</button><button onClick={e=>{e.stopPropagation();setDId(null)}} style={{background:C.sf,border:"1px solid "+C.bd,color:C.tD,cursor:"pointer",fontSize:11,padding:"3px 8px",borderRadius:6}}>No</button></span>:<button onClick={e=>{e.stopPropagation();setDId(r.id)}} style={{background:"none",border:"none",color:C.r,cursor:"pointer",fontSize:12,fontWeight:600,opacity:.7}}>Elim</button>}</div>}]:[])] } data={fl}/>
  </div>;
}
