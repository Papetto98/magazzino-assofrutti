...
| Movimenti (Entrata/Uscita/Trasferimento/Trasformazione) | Sì | Sì |
| Uscita rapida + selezione multipla + assegnazione (Giacenze) | Sì | Sì |
| Annulla movimento (Storico Mov.) | Sì | Sì |
| Export Excel / Stampa / Resoconto PDF-Excel / Etichette QR / Scansione | Sì | Sì |
| Contratti: crea/modifica/elimina | No | Sì |
| Lotti: modifica/elimina | No | Sì |
| Utenti: crea/gestisci ruoli | No | Sì |

## 12. MOVIMENTI — DETTAGLI

- **Entrata**: form completo (tutti i campi qualità incluso RT) + checkbox conto lavoro + banner **"Stampa etichetta"** del lotto appena creato.
- **Uscita**: full-page, selezione multipla, **DDT obbligatorio**, pulsante "Registra Uscita".
- **Trasferimento**: full-page **multi-lotto** (`handleMultiTransfer`), qta per lotto, destinazione unica, **DDT obbligatorio**. Parziale → crea split tracciato; totale → sposta magazzino.
- **Trasformazione**: vedi §8.
- **DDT obbligatorio** su TUTTE le uscite (Movimenti + uscita rapida Giacenze) e trasferimenti; visibile come **colonna DDT in Storico Movimenti** e nell'export. Entrata e Trasformazione senza DDT manuale.
...
- **Scansione** (`Scanner`, html5-qrcode in **import dinamico** per non appesantire il bundle): fotocamera, richiede HTTPS (Vercel ok) e permesso browser. `parseQR(txt)` valida il prefisso e ritorna l'id.
  - **Lettura** (Giacenze → "Scansiona QR") → apre la **scheda del lotto** (qualità, magazzino, contratto, tracciabilità).
  - **Per-agire** (Uscita/Trasferimento → "Scansiona QR") → aggiunge il big bag alla **selezione** con disponibilità precompilata. **ABC/trasformazione esclusi** dalla scansione-per-agire.
